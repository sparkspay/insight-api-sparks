...
<div align="center">
  <strong>A sparks blockchain REST and WebSocket API Service</strong>
</div>
<br />
<div align="center">
  <!-- Stability -->
...
This is a backend-only service. If you're looking for the web frontend application, take a look at https://github.com/sparkspay/insight-ui.
...
- [Resources](#resources)
- [License](https://github.com/sparkspay/insight-api-sparks/blob/master/LICENSE)

## Getting Started

```bashl
npm install -g sparkscore-node@latest
bitcore-node-sparks create mynode
cd mynode
...
- [Bitcore Node sparks 3.x](https://github.com/sparkspay/sparkscore-node)

**Note:** You can use an existing sparks data directory, however `txindex`, `addressindex`, `timestampindex` and `spentindex` needs to be set to true in `sparks.conf`, as well as a few other additional fields.
...
```
With all the configuration options available: https://github.com/sparkspay/insight-api/blob/master/lib/ratelimiter.js#L10-17
...
### Block
```
  /insight-api-sparks/block/[:hash]
  /insight-api-sparks/block/0000000006e7b38e8ab2d351239019c01de9a148b5baef58cfe52dfd9917cedc
```

### Block Index
Get block hash by height
```
  /insight-api-sparks/block-index/[:height]
  /insight-api-sparks/block-index/0
```
This would return:
```
{
...
### Raw Block
```
  /insight-api-sparks/rawblock/[:blockHash]
```
...
```
  /insight-api-sparks/blocks?limit=3&blockDate=2017-04-22
```
...
### Transaction
```
  /insight-api-sparks/tx/[:txid]
  /insight-api-sparks/tx/ebdca263fe1c75c8609ce8fe3d82a320a0b3ca840f4df995883f5dab1b9ff8d9
  /insight-api-sparks/rawtx/[:rawid]
  /insight-api-sparks/rawtx/ebdca263fe1c75c8609ce8fe3d82a320a0b3ca840f4df995883f5dab1b9ff8d9
```

### Address
```
  /insight-api-sparks/addr/[:addr][?noTxList=1][&from=&to=]
  /insight-api-sparks/addr/ybi3gej7Ea1MysEYLR7UMs3rMuLJH5aVsW?noTxList=1
  /insight-api-sparks/addr/yPv7h2i8v3dJjfSH4L3x91JSJszjdbsJJA?from=1000&to=2000
```

### Address Properties
```
  /insight-api-sparks/addr/[:addr]/balance
  /insight-api-sparks/addr/[:addr]/totalReceived
  /insight-api-sparks/addr/[:addr]/totalSent
  /insight-api-sparks/addr/[:addr]/unconfirmedBalance
```
The response contains the value in Satoshis.

### Unspent Outputs
```
  /insight-api-sparks/addr/[:addr]/utxo
```
Sample return:
```
[
...
GET method:
```
  /insight-api-sparks/addrs/[:addrs]/utxo
  /insight-api-sparks/addrs/ygwNQgE5f15Ygopbs2KPRYMS4TcffqBpsz,ygw5yCtVkx3hREke4L8qDqQtnNoAiPKTSx/utxo
```

POST method:
```
  /insight-api-sparks/addrs/utxo
```
...
```
  /insight-api-sparks/txs/?block=HASH
  /insight-api-sparks/txs/?block=000000000814dd7cf470bd835334ea6624ebf0291ea857a5ab37c65592726375
```
### Transactions by Address
```
  /insight-api-sparks/txs/?address=ADDR
  /insight-api-sparks/txs/?address=yWFfdp9nLUjy1kJczFhRuBMUjtTkTTiyMv
```

### Transactions for Multiple Addresses
GET method:
```
  /insight-api-sparks/addrs/[:addrs]/txs[?from=&to=]
  /insight-api-sparks/addrs/ygwNQgE5f15Ygopbs2KPRYMS4TcffqBpsz,ygw5yCtVkx3hREke4L8qDqQtnNoAiPKTSx/txs?from=0&to=20
```

POST method:
```
  /insight-api-sparks/addrs/txs
```
...
GET method:
```
  /insight-api-sparks/gobject/info
```
...
GET method:
```
  /insight-api-sparks/gobject/count
```
...
GET method:
```
  /insight-api-sparks/gobject/list/proposal (or /insight-api-sparks/gobject/list)
```
...
GET method:
```
  /insight-api-sparks/gobject/list/trigger
```
...
GET method:
```
  /insight-api-sparks/gobject/get/[:hash]
  /insight-api-sparks/gobject/get/b6af3e70c686f660541a77bc035df2e5e46841020699ce3ec8fad786f7d1aa35
```
...
GET method:
```
  /insight-api-sparks/gobject/check/[:hexData]
  /insight-api-sparks/gobject/check/5b5b2270726f706f736[..]
```
...
GET method:
```
  /insight-api-sparks/gobject/deserialize/[:hexData]
  /insight-api-sparks/gobject/deserialize/5b5b2270726f706f736[..]
```

Sample output:
```
{
  "result":"[[\"proposal\",{\"end_epoch\":1519848619,\"name\":\"ghijklmnopqrstuvwxyz01234567891519097947\",\"payment_address\":\"yik5HAgVAgjH1oZKjcDfvcf22bwBNbSYzB\",\"payment_amount\":10,\"start_epoch\":1519097947,\"type\":1,\"url\":\"https://www.sparkscentral.org/p/test_proposal_1519097947\"}]]",
  "error":null,
  "id":78637
}
```
...
GET method:
```
  /insight-api-sparks/gobject/votes/current/[:hash]
  /insight-api-sparks/gobject/votes/current/fbda8cdc1f48917f53b7d63fbce81c85d6dedd3d0e476e979926dfd154b84034
```
...
GET method:
```
  /insight-api-sparks/governance/budget/[:blockIndex]
  /insight-api-sparks/governance/budget/79872
```
...
POST method:
```
  /insight-api-sparks/gobject/submit
```
...
### Masternodes List
```
  /insight-api-sparks/masternodes/list
```
### Validate Masternode
```
  /insight-api-sparks/masternodes/validate/[:payee]
  /insight-api-sparks/masternodes/validate/yRuALkPpeYpTgxdNn2L5YgGktASJYDYPAo
```
...
```
  /insight-api-sparks/sync
```

### Live Network P2P Data Sync Status
```
  /insight-api-sparks/peer
```

### Status of the Bitcoin Network
```
  /insight-api-sparks/status?q=xxx
```
...
### Utility Methods
```
  /insight-api-sparks/utils/estimatefee[?nbBlocks=2]
```
...
- The response for the `/sync` endpoint does not include `startTs` and `endTs` as the sync is no longer relevant as indexes are built in sparksd.
- The endpoint for `/peer` is no longer relevant connection to sparksd is via ZMQ.
...
The `/status?q=getTxOutSetInfo` method has also been removed due to the query being very slow and locking sparksd.
...
- (Medium)[How to setup a sparks Instant-Send Transaction using Insight API?????????The comprehensive way](https://medium.com/@obusco/setup-instant-send-transaction-the-comprehensive-way-a80a8a0572e)
