...
BlockController.prototype.checkBlockHash = function(req, res, next) {
  ...
  if (hash.length < 64 || !isHexadecimal(hash)) {
    this.node.services.sparksd.getBlockHeader(parseInt(hash), function(err, info) {
      if (err) {
...
BlockController.prototype.blockHeader = function(req, res, next) {
	...
	self.node.services.sparksd.getBlockHeader(hash, function(err, info) {
		if (err) {
			return self.common.handleErrors(err, res);
		}
...
BlockController.prototype.blockHeaders = function(req, res, next) {
	...
	if(blockIdentifier.length===64){
		...
		var result = self.node.services.sparksd.getBlockHeaders(hash, cb, nbOfBlockToFetch);
	}else{
		var height = blockIdentifier;
		var result = self.node.services.sparksd.getBlockHeaders(height, cb, nbOfBlockToFetch);
	}
...
BlockController.prototype.block = function(req, res, next) {
  ...
  if (blockCached) {
    blockCached.confirmations = self.node.services.sparksd.height - blockCached.height + 1;
...
BlockController.prototype._normalizePrevHash = function(hash) {
  // TODO fix sparkscore to give back null instead of null hash
...
BlockController.prototype.blockIndex = function(req, res) {
  ...
  this.node.services.sparksd.getBlockHeader(parseInt(height), function(err, info) {
    if (err) {
      return self.common.handleErrors(err, res);
    }
    res.jsonp({
...
BlockController.prototype._getBlockSummary = function(hash, moreTimestamp, next) {
  ...
  if (summaryCache) {
    ...
    self.node.services.sparksd.getRawBlock(hash, function(err, blockBuffer) {
      if (err) {
        return next(err);
      }

      var br = new sparkscore.encoding.BufferReader(blockBuffer);
      ...
      // would require changes in sparkscore-node.
      var header = sparkscore.BlockHeader.fromBufferReader(br);
      var info = {};
      var txlength = br.readVarintNum();
      info.transactions = [sparkscore.Transaction().fromBufferReader(br)];

      self.node.services.sparksd.getBlockHeader(hash, function(err, blockHeader) {
        if (err) {
          return next(err);
        }
        ...
        var confirmations = self.node.services.sparksd.height - height + 1;
...
BlockController.prototype.list = function(req, res) {
  ...
  self.node.services.sparksd.getBlockHashesByTimestamp(lte, gte, function(err, hashes) {
    if(err) {
      return self.common.handleErrors(err, res);
    }
...
BlockController.prototype.getPreviousBlock = function(prevHash, cb) {
  ...
  if(prevHash===null) cb(null,null);
      ...
      self.node.getBlock(prevHash, function (err, block) {
          ...
          self.node.services.sparksd.getBlockHeader(prevHash, function (err, info) {
              if (err) {
...
BlockController.prototype.getBlockReward = function(prevHash, cb) {
  ...
  self.getPreviousBlock(prevHash, function(err, info) {
    ...
    if(info===null){
        ...
        cb(null, parseFloat("650000".toString(10)).toFixed(8));
    }
    else{
    ...
    if(info===null){
        //Genesis reward is 50.
        //cb(null, parseFloat("50".toString(10)).toFixed(8));
        cb(null, parseFloat("18".toString(10)).toFixed(8));
    } else if (info===1) {
        cb(null, parseFloat("650000".toString(10)).toFixed(8));
    }
    else{
